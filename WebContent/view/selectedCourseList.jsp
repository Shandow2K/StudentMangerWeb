...
<script type="text/javascript">
 	...
 	$(function() {
		$.ajax({
				type : "post",
				url  : "SelectedCourseServlet?method=isSelectCourseDate",
				data : null,
				contentType: "application/x-www-form-urlencoded; charset=utf-8",
				success : function(msg,msg1,msg2) {
					if (msg == "fail") {
						$.messager.alert("消息提醒","不可选课!","info");
						}
					else {
						$("#selectCourseDate").append(msg);	
		...
		$('#dataList').datagrid(
						{
							...
							fit : true,//自动大小 
							method : "post",
							url : "SelectedCourseServlet?method=SelectedCourseList&t="
									+ new Date().getTime(),
							idField : 'id',
			...
			$("#courseList").combobox(
					{
						...
						editable : false, //不可编辑
						method : "post",
						url : "CourseServlet?method=CourseList&t="
								+ new Date().getTime() + "&from=combox",
		...
		$("#delete").click(
						function() {
							...
							if (selectRow == null) {
								...
								var id = selectRow.id;
								$.messager.confirm("消息提醒","将删除与课程相关的所有数据，确认继续？",
												function(r) {
													if (r) {
														$.ajax({
																	type : "post",
		...
		//下拉框通用属性
		$("#add_studentList, #add_courseList,#studentList,#courseList")
				.combobox({
					width : "200",
					height : "30",
					valueField : "id",
		...
		$("#search-btn").click(
				function() {
					$('#dataList').datagrid('load',
							{
								studentid : $("#studentList").combobox(
										'getValue') == '' ? 0 : $(
										"#studentList").combobox('getValue'),
								courseid : $("#courseList")
										.combobox('getValue') == '' ? 0 : $(
										"#courseList").combobox('getValue')
							});
				});
	});
...
<body>
	...
	<div id="addDialog" style="padding: 10px">
		<form id="addForm" method="post">
			<table cellpadding="8">
			    <tr>
			    <td style="width: 60px">选课时间:</td>
				<td id="selectCourseDate" style="width: 200px"></td>
