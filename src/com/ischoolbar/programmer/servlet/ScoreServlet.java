...
public class ScoreServlet extends HttpServlet {
	...
	private void exportScore(HttpServletRequest request,
		...
		try {
			...
			createRow.createCell(2).setCellValue("课程编号");
			createRow.createCell(3).setCellValue("课程");
			createRow.createCell(4).setCellValue("成绩");
			createRow.createCell(5).setCellValue("备注");
			//实现将数据装入到excel文件中
			int row = 1;
			for(Map<String, Object> entry:scoreList){
				createRow = createSheet.createRow(row++);
				createRow.createCell(0).setCellValue((int) entry.get("studentId"));
				createRow.createCell(1).setCellValue(entry.get("studentName").toString());
				createRow.createCell(2).setCellValue((int) entry.get("courseId"));
				createRow.createCell(3).setCellValue(entry.get("courseName").toString());
				createRow.createCell(4).setCellValue(new Double(entry.get("score")+""));
				createRow.createCell(5).setCellValue(entry.get("remark")+"");
			}
